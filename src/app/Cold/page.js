...
function Cold() {
    ...
    useEffect(() => {
        const fetchLead = async () => {
            try {
                let url;
                if (userrole === "Admin") {
                    url = `/api/Lead/get?page=${currentPage}&limit=50`;
                }
    ...
    }, [userrole, userid, selectedTag, selecteduser, currentPage]);
    ...
    const currentLeads = Leadss.slice(indexOfFirstLead, indexOfLastLead);

    // Change page
    const nextPage = async () => {
        if (currentPage < Math.ceil(totalLeads / leadsPerPage)) {
    ...
    const leadcountf = (value) => {
        setCurrentPage(1);
        setleadsPerPage(parseInt(value, 10)); // Parse the value to an integer
    };
    ...
    const leadElementHeight = 200; // Height of each lead element in pixels
    const visibleLeadRange = {
        start: Math.max(0, (currentPage - 1) * leadsPerPage),
        end: Math.min(Leadss.length, currentPage * leadsPerPage)
    };

    const handleScroll = (event) => {
        const { scrollTop, clientHeight, scrollHeight } = event.target;
        const scrollPosition = scrollTop + clientHeight;
        const totalHeight = Leadss.length * leadElementHeight;
    ...
    const handleSubmission = () => {
        if (selectedLeads.length === 1) {
            ...
            if (leadId) {
                window.location.href = `/Invoice/create?leadId=${leadId}`;
            } else {
    ...
    const filteredLeads = Leadss.filter((lead) =>
    ...
    return (
        <RootLayout>
            ...
            <div className='container-fluid'>
                <div className='row mt-5' onScroll={handleScroll} style={{ height: '500px', overflowY: 'scroll' }}>
                    {searchTerm ? (
                        ...
                        Leadss.slice(visibleLeadRange.start, visibleLeadRange.end).map((currentLead) => (
                            <div className='col-md-4 card position-relative' key={currentLead._id} onMouseDown={() => handleLongPressStart(currentLead._id)} onMouseUp={handleLongPressEnd} onMouseLeave={handleLongPressEnd} onTouchStart={() => handleLongPressStart(currentLead._id)} onTouchEnd={handleLongPressEnd} style={{ background: selectedLeads.includes(currentLead._id) ? 'lightblue' : '#fff' }}>
                ...
                <div className="row">
                    <div className="col-sm-12 col-md-12">
                        <div className="dataTables_info" id="datatable_info" role="status" aria-live="polite">
                            Showing {indexOfFirstLead + 1} to {Math.min(indexOfLastLead, Leadss.length)} of {Leadss.length} entries
                        </div>
                    </div>
                    <div className="col-sm-12 col-md-12">
                            ...
                            <ul className="pagination pagination-rounded">
                                ...
                                {Array.from({ length: Math.ceil(totalLeads / leadsPerPage) }, (_, i) => (
                                    <li key={i} className={`paginate_button page-item ${i + 1 === currentPage ? 'active' : ''}`}>
                                        <a href="#" className="page-link" onClick={() => setCurrentPage(i + 1)}>
                                            {i + 1}
                                        </a>
                                    </li>
                                ))}
                                <li className={`paginate_button page-item next ${indexOfLastLead >= totalLeads ? 'disabled' : ''}`}>

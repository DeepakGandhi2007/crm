...
async function parseCSVAndSaveToDB(csvData, LeadStatus, userIdyoken) {
    ...
    for (const lead of parsedData) {
        ...
        if (!marketingtag) {
            console.log(`Marketing tag with name ${lead.form_name} not found, creating new tag...`);
        ...
        if (existingLead) {
            console.log(`Lead with phone number ${lead.Phone} and tag ${marketingtagId} already exists, skipping...`);
            continue;
        }
        let dldtagId = "";
        if (lead.tags) {
            ...
            if (!dldtag) {
                console.log(`DLD tag with name ${lead.tags} not found, skipping lead...`);
                continue;
            }
            dldtagId = dldtag._id;
        }
        ...
        if (!user) {
            console.log(`User with username ${lead.agent} not found, skipping lead...`);
            continue;
        }
